...
function createCertSlide(cert, index) {
  ...
  content.innerHTML = `<strong>${cert.title}</strong><br>${cert.issuer}<br><em>${cert.year}</em>`;
  card.appendChild(content);
  certStack.appendChild(card);
}

certifications.forEach((cert, index) => {
  createCertSlide(cert, index);
});

const certCards = document.querySelectorAll('#cert-stack .file-card');

function showCertSlide(index) {
  certCards.forEach((card, i) => {
    card.classList.toggle('active', i === index);
  });
  certDesc.textContent = `${certifications[index].title} (${certifications[index].year}) by ${certifications[index].issuer}`;
}

function nextCert() {
  certCurrent = (certCurrent + 1) % certCards.length;
  showCertSlide(certCurrent);
}

function prevCert() {
  certCurrent = (certCurrent - 1 + certCards.length) % certCards.length;
  showCertSlide(certCurrent);
}
